<h1>Reference Guide -  Web Development</h1>

This is a reference guide for web development written with love and broken english, for myself, by myself, for educational purposes and for fun. <br> <br>
...
[Whats's built-in](#what-is-vscode) ◦ [Extensions](#useful-extensions-in-vscode) ◦ [Shortcuts](#useful-shortcuts-in-vscode) ◦ [Snippets](#snippets-in-vscode) ◦ [Terminal](#terminal-in-vscode) ◦ [Intellisense](#intellisense-in-vscode) ◦ [Settings and formatting of code](#settings-and-formatting) 
...
`Open with Code` - start form ui by right-clicking on a folder in windows explorer 

<p align="right"><a id="what-is-vscode" href="#table-of-content">↩ Back To Top</a></p>

### **[What's built-in?](#)**

Here are some of default build-in features in VSCode:
-   **JavaScript:** Language run natively in browsers, used for web dev with syntax similar to Java.
-   **TypeScript**: Extended Javascript. Adds strict typing syntax and more strict coding rules to JavaScript.
...
Many extensions are getting integrated into VSCode with each release. Yor can download extensions from [marketplace](https://marketplace.visualstudio.com/).

| Type | Name | Details |
| :-- | :-- | :-- |
| Spelling (def) | `Code Spell Checker` | Corrects spelling mistakes and proposes corrections. |
| Snippets (def) | `Easy Snippet` | A handy snippet manager with icon in explorer bar making it easy to add, edit and delete own snippets.<br><br> Note! Snippets in vscode are saved locally in `user..\Code\User\snippets\` as `.code-snippet` (by vscode) or `.json`(by Easy Snippet) files. This extension handles only .`json` files and the filename specifies language/scope where it is applied i.e. snippets in javascript.json are for javascript language.It is recommended to use this extension to save language defined snippets, and use vscode with its .code-snippet files to define global snippets. When "scope" is missing in vscode snippets then the snippet is applied in all languages.|
| Intellisense | `Tagnite` | Adds suggestions smartly guesing what you want to write. Works in all types of files. |
| CSS Intellisense (def) | `HTML CSS Support` | Extension adding suggestions in intellisense for css classes defined in linked and external files. To add locally in project define rules in `.vscode/settings.json` <br><br> Works in **Vue** files listing classes defined in `<styles>` par, when "vue" language is added in extension settings (_settings.json_): `"css.enabledLanguages": ["html", "vue"]`. <br><br> Note! CSS content linked in html by `<link rel="stylesheet" href="CSS.css">` (or embedded in `<style>` tag) is used in suggestions by default. To use external files (not linked in html files) then add location of those css files in extension setting (_setting.json_): `"css.styleSheets": [/folder/file.css]` <br><br> There are css frameworks as bootstrap, mdb, fontawsome that are downloaded into `node_modules`. Then location of compiled css can be linked by `"css.styleSheets": [/node_modules/bootstrap/dist/css/bootstrap.css]`. You can also link to external cdn `"css.styleSheets": ["https://cdn.jsdelivr.net/npm/bootstrap@4.5.3/dist/css/bootstrap.min.css"]` <br><br> Note! If css files are generated with a bundler when building for i.e. production the final css file is placed in some `dist` folder. As generated css file-name changes for each build, it is impossible to link to it with `css.styleSheet` setting option. <br><br>  There exists another extension called **IntelliSense for CSS class names in HTML** where linking to whole folders is possible. Then this extension can be used. |
| CSS Intellisense (def) | `Intellisense for CSS class names in HTML` | Extension adding suggestions in intellisense for css classes defined in linked and external files and folders. <br><br> Note! CSS content linked in html by `<link rel="stylesheet" href="CSS.css">` (or embedded in `<style>` tag) is used in suggestions by default. <br><br> This extension adds possibility to link css files from folders! In Vue it imports all suggestions but not those in `<styles>` parts. |
| CSS Intellisense | `HTML to CSS autocompletion` | Add suggestions in css for classes that are defined in html files. Searches html in workspace or linked and proposes in .css files. <br><br>Does not work in vue files <br><br> This might be usefull in smaller projects not build by bundlers. |
| CSS Intellisense (def) | `CSS Peak` | Adds Go-To-Definition (ctrl hoover click or **ctrl alt click** to open in side window) and Peek (rightclick->Peek). In Vue files it links with file definitions from other files. Good for plain HTML/CSS project though. Go to (F10) is really good. If wish to enter and click in peak add "editor.stablePeek": true, in order to stay in the peak (otherwise it navigates to definition) |
| JS Intellisense (def) | `Node.js Modules Intellisense` | Populates intellisense with names of modules in import statements. I.e `const http = require('ht')` will show http and https |
| Path Intellisense | `Path Intellisense` | Searches workspace for files and propose them when writing src="`./`" href="`./`" or form "`./`". This functionality is build-in for HTML/JS files in VSCode but works for all types of files out-of-the-box with this extension. Vue is handles by vue extension.. <br> Set  Auto Slash After Directory and Auto Trigger Next Suggestion to add a `/` char and auto open list with proposals (bit bugg shows all kind of crap proposals though). |
| Intellisense (not used) | `Auto import` | Automatically finds, parses and adds to IntelliSense code actions proposals with code completion with available import actions. |
| Efficiency | `Bookmarks` | Set bookmarks in code and then with shortcuts jumt to them. Handy for fat navigation in big files. |
| Web dev | `MatchIt` | Automatically navigates between focused, matching brackets or tags. By default shortcut `Ctrl Alt §` is added. |
| Syntax | `Prettier` | Formatter that is populat for all languages. Works out of the box. |
| Syntax | `DotENV` | Code formatter for .env files. Works out of the box. |
| Coding (not necessary) | `Auto Rename Tag` | Renames matching html tags. Works for all file types. <br><br>VSCode offers this feature by default (enabled in Settings by `"editor.linkedEditing": true`) but then it works only for html files (not vue). But with vue extension Volar (not Vetur) it work in Vue files as well. So redundant. |
| Coding (not necessary) | `Auto Close Tag` | Closes matching html tags. Works for all file types .html as well as .vue files. <br><br>VSCode offers this feature by default (enabled in Settings by `"editor.linkedEditing": true`) but then it works only for html files (not vue). But with vue extension Volar (not Vetur) it work in Vue files as well. So redundant. |
| Efficiency | `Live Preview` | Fast debug/dev server. Can run in VSCode internal window. Easy to switch to external browser. Load changes on save only or instantly. Prints logs in terminal window. Icon in top/right to quickly start. (Useing this instead of Live Server with Browser Preview extensions.) |
| Efficiency | `Todo Tree` | Show all your TODOs in a tree with icon in the side bar. Very usefull. |
| Syntax | `ESLint` | Analyzes your code, finds and fix problems in your project files. Needed for error highlighting etc together with installation of needed development dependencies. How to configure for vue [here](https://medium.com/@gogl.alex/how-to-properly-set-up-eslint-with-prettier-for-vue-or-nuxt-in-vscode-e42532099a9c). Rules [here](https://eslint.org/docs/rules/). |
| PostCSS | `PostCSS Intellisense and Highlighting` | Adds syntax highlighting and intellisense in .css with postcss syntax. Emmet to work can be added with following in setting.json file: "emmet.includeLanguages": {"postcss": "css"} |
| PostCSS | `PostCSS Language Support` | Syntax highlighting and emmet. Changes .css to be associated with postCSS Language. Must add "emmet.includeLanguages": {"postcss": "css"} to get css emmet and intellisense.|
| Vue (not necessary)| `Vetur` | For vue.js development with syntax-highlighting, formatting, EMMET, IntelliSense etc. Included auto-formatting might interfere with "Prettier" formatter. Suggested to install Prettier first, customize it for HTML and JS later install Vetur. Vetur uses Prettier internally as main formatter (no extension is needed) for HTML and JS features within vue files. Disable html attribute wrapping by adding in Setting for `Default Formatter Options` following `"js-beautify-html": {"wrap_attributes": "auto"}` and use `js-beautify-html` formatter as `Vetur: Default Formatter: HTML`. Read more [here](#recommended-customization-of-vscode). Documentation [here](https://vuejs.github.io/vetur/). |
| Vue | `Volar` | Reccomented all inclusive extension for Vue 2 and Vue 3|
| Vue | `Vue VSCode Snippets` | Snippets to supercharge you workflow. Snippets like `vmethod` and `vdata` expands to data and method js-script code. |
| Git | `Graph` | Visualizes branches history in separate window nicely. Shows handy "Git Graph" button on the status bar. |
| Git | `GIT Lens` | Extensive Git insights with own sidebar in explorer. Good file history comparison. Can see in code who made changes and commit lables. Packed with features! |
| Git | `History` | This tool draws nice file history diagrams. |
| Git | `Gitignore` | Pull in .gitignore files |
| Markdown | `All in One` | Features like "export .md to .html", build tables of contents and more |
| Markdown | `Shortcuts` | Adds shortcuts and clickable icons on top bar, handy for highlighting, adding images and styling text. |
| Markdown | `Markdown preview enhanced` | Improves how the preview looks. |
...
Add from GUI from `settings -> Kkeyboard shortcuts`. Changes are in background written to a file named **keybinding.json** and this file can be opened from command palette: `alt shift p` -> `Keyboard Shortcuts (JSON)`.

Our customized keybinding.json file (with keybinding below) can be downloaded [from TODO](https://gist.github.com/428da4b8a35f5c1b28479f8629aba97d.git).
...
To see a **list of all snippets** for a given language (file extension) **from** such a file open Commad Palette `Snippets: Insert Snippet` view list of own defined snippets,default VsCode and sippets added extesions. It is easy from this list t **disable** them from being shown in itellisese.

You can **create own snippet**s. They can be global aplied in all workspace, or local just for current workspace. Create VSCode snippets from Command Palette - `Snippets: Configure User Snippets` and choose `new global snippets...` or `new snipper for ...`.

Snippets created with VSCode get extension `.code-snippets`. Extension like "Easy Snippet" save the file with extension `.json` and use filename as language they apply for.

```javascript
// GLOBAL save in
C:\Users\<uset>\AppData\Roaming\Code\User\snippets
 
 // VSCode snippets
 global.code-snippets
 
 //"Easy Snippets" snippets
 javascript.json
 css.jason
...
```javascript
// Example of a snippet in `.code-snippets` file.
{
  ...
  "hello": {
...
VSCode comes with an integrated terminal window. By default it starts in current working directory that is **the root** of your opened workspace. Note, you can open **an external terminal window** with command `Ctrl + Shift + C`.  
...
**[Settings](#)**  
VSCode settings can be viewed and changed in a GUI from `File > Preferences > Settings`. 

Setings are saved in 3 types of setting files and you can open those files by searchiin in command palette for them:
...
Settings saved here in file `root/.vscode/`settings.json` are valid only for given workspace. This file is possible to edit and its contend overwrites _user/settings.json_ and _defaultSettings.json_.
...
**[Formatting](#)**  
For HTML and JavaScript there are default formatters shipped with VSCode but at writing moment CSS do not have any formatter. An extension must be installed to format CSS Same for Vue. There are popular extensions like **Prettier** (supporting pretty much formatting of all languages, including CSS) and **Volar** for Vue. Those extensions overwrite native VScode formatting by adding formatting with syntax highlighting and styles on their own. Note that prettier have their own opinionated configuration thaat can t be changed but there are some settings like line length, semicollon in ens of line or usage of double/single quotes.

By default shortcuts `Ctrl+K Ctrl+F` formats only the selection and `Ctrl+Alt+F` formats whole document but by enabling `Editor: Format on save` in VSCode settings the formater will trigged for each save. Recommended.
...
<p align="right"><a id="virtual-environments" href="#table-of-content">↩ Back To Top</a></p>
